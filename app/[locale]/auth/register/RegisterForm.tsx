...
export default function RegisterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (password.length < 8) { setError(t('pwMinLength')); return; }
  ...
  const passwordStrength = () => {
    ...
    const labels = [t('pwStrengthWeak'), t('pwStrengthFair'), t('pwStrengthGood'), t('pwStrengthStrong')];
    const colors = ['#CC6E6E', '#CC9E6E', '#A4C46E', '#6ECC96'];
    return { label: labels[score - 1] ?? t('pwStrengthVeryWeak'), color: colors[score - 1] ?? '#CC6E6E', score };
  };
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div>
        ...
        <input type="text" required minLength={2} maxLength={120} autoComplete="name" value={name} onChange={(e) => setName(e.target.value)} className="field" placeholder={t('namePlaceholder')} disabled={loading} />
      </div>
      <div>
      ...
      <div>
        ...
        <input type="password" required minLength={8} autoComplete="new-password" value={password} onChange={(e) => setPassword(e.target.value)} className="field" placeholder={t('passwordPlaceholder')} disabled={loading} />
        {strength && (
      ...
      <p style={{ fontSize: 11, color: 'var(--text-muted)', textAlign: 'center', lineHeight: 1.5 }}>{t('terms')}</p>
    </form>
  );
}
