...
import { Mail, Hash, Phone, MapPin, Pencil, Save, X, LogOut } from 'lucide-react';
...
export default function ProfileClient() {
  const { mounted, profile, isLoading, editing, setEditing, form, setForm, msg, isPending, handleEdit, handleSave, handleLogout } = useProfileActions();
  ...
  return (
    <div className="container-wide py-12 pb-20 flex-1 px-6">
      <div className="mb-8">
        <p className="text-[11px] tracking-[0.14em] uppercase text-primary mb-2 font-semibold">CUENTA</p>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-[240px_1fr] lg:grid-cols-[300px_1fr]">
        ...
        <div className="flex flex-col gap-4">
          <Card className="overflow-hidden">
            ...
            >
              <div className="absolute -bottom-9 left-6 border-[3px] border-card overflow-hidden" style={{ borderRadius: 16, width: 72, height: 72 }}>
            ...
            <CardContent className="pt-14 pb-6 px-6">
              ...
              {msg && (
                <p className={cn('text-[12px] mb-3', msg.includes('!') ? 'text-[#6ECC96]' : 'text-destructive')}>{msg}</p>
              )}
              <Separator className="mb-4" />
              <p className="text-[11px] text-muted-foreground leading-relaxed">{profile.email}</p>
            </CardContent>
          </Card>
            ...
            {editing ? (
              <div>
                <div className="flex items-center justify-between mb-6">
                  <h3 className="font-normal" style={{ fontFamily: 'var(--font-display)', fontSize: '1.2rem' }}>Editar información</h3>
                  <Button variant="ghost" size="sm" onClick={() => setEditing(false)} className="flex items-center gap-1.5 text-muted-foreground">
                    <X size={13} /> Cancelar
                  </Button>
                </div>
                ...
                <div className="flex gap-2 mt-7">
                  <Button size="sm" onClick={handleSave} disabled={isPending} className="flex items-center gap-1.5">
                    <Save size={12} />
                    {isPending ? 'Guardando…' : 'Guardar cambios'}
                  </Button>
                  <Button variant="ghost" size="sm" onClick={() => setEditing(false)} className="flex items-center gap-1.5">
                    <X size={12} /> Cancelar
                  </Button>
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-6">
                  <h3 className="font-normal" style={{ fontFamily: 'var(--font-display)', fontSize: '1.2rem' }}>Información personal</h3>
                  <Button variant="outline" size="sm" onClick={handleEdit} className="flex items-center gap-1.5 text-[11px]">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-12">
                  {infoRows.map(({ icon, label, value }, idx) => (
                    <div key={label}>
                      ...
                      {idx < infoRows.length - 1 && <Separator className="sm:hidden" />}
                    </div>
                  ))}
                </div>
              </div>
